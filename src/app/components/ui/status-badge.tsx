...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  ...
  if (normStatus === 'approved' || normStatus === 'technical_approved' || normStatus === 'commercial_approved' || normStatus === 'shortlisted' || normStatus === 'active' || normStatus === 'verified' || normStatus === 'delivered' || normStatus === 'paid' || normStatus === 'receiving completed' || normStatus === 'goods received') {
    bgClass = "bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400";
    displayLabel = normStatus === 'paid' ? 'Paid' : normStatus === 'active' ? 'Active' : normStatus === 'verified' ? 'Verified' : normStatus === 'delivered' ? 'Delivered' : normStatus === 'receiving completed' ? 'Receiving Completed' : normStatus === 'goods received' ? 'Goods Received' : normStatus === 'technical_approved' ? 'Technical Approved' : normStatus === 'commercial_approved' ? 'Commercial Approved' : 'Approved';
